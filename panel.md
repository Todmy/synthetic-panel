---
name: panel
description: Evaluate content through a synthetic audience panel — personas × contexts × metrics matrix. Self-configures on first run.
...
After the initial evaluation, you stay in conversational mode for follow-up questions, revised drafts, calibration, and persona/context management.

---

## Step 0: Detect Mode

Check what the user wants based on `$ARGUMENTS` and file state:

**Init needed**: If `panel/data/config.json` does NOT exist (use Glob to check):
- Print: "Panel not configured yet. Starting setup."
- Jump to **Init Wizard** below.

**Setup requested**: If `$ARGUMENTS` is "setup" or "reconfigure":
- Print: "Reconfiguring panel. Current calibration data will be preserved but may become invalid."
- Jump to **Init Wizard** below.

**Normal evaluation**: If `panel/data/config.json` exists AND `$ARGUMENTS` contains content:
- Proceed to Step 1.

**No arguments, configured**: If `panel/data/config.json` exists but `$ARGUMENTS` is empty:
- Print brief help: "Usage: `/panel <content or file path>` to evaluate. `/panel setup` to reconfigure."
- Stop.
