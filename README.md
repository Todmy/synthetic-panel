...
> **Requires [Claude Code](https://docs.anthropic.com/en/docs/claude-code) (CLI, desktop app, or IDE extension).** The evaluation engine spawns 5 parallel agents — a feature only available in Claude Code. This skill will not work in ChatGPT, Claude.ai web chat, or other LLM interfaces without agent spawning support.
...
**5. Calibration** — Feed real performance data after each evaluation. The panel parses numbers from natural language, computes a `weighted_score`, and tracks synthetic↔real correlation:

```
weighted_score = reactions×1 + comments×5 + reposts×10 + saves×2
```

Weights are configurable via `action_weights` in `panel/data/config.json`. Minimum 4 posts with both synthetic and real scores needed for correlation. Ask "How accurate is the panel?" to get a Pearson correlation report (`r > 0.7` = strong, `0.5–0.7` = moderate, `< 0.5` = needs more data).
...
Each operation consumes tokens. Numbers below are upper bounds measured on real runs with 20 personas × 15 contexts.

| Operation | Tokens (up to) | What happens |
|-----------|---------------|--------------|
| **Setup (Init Wizard)** | ~150K | Reads presets, generates 4 JSON files (config, personas, contexts, metrics), creates directories |
| **One evaluation run** | ~400K | 5 parallel agents (~60K each) + orchestration, aggregation, report generation |
| **Calibration (per post)** | ~60K | Single agent evaluates all personas × contexts, computes reach/buyer scores |

**Estimated cost per evaluation run:**
- Claude Opus: up to ~$8
- Claude Sonnet: up to ~$2

**Full calibration (12 posts):** up to ~720K tokens total (~$12 Opus, ~$3 Sonnet).

Token usage scales with panel size. Fewer personas or contexts = fewer tokens.
